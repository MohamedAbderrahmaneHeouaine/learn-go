...
func main(){
	...
	v1.Post("/feed_follows",apiCfg.middlewareAuth(apiCfg.handlerFeedFollowCreate))
	v1.Get("/feed_follows",apiCfg.middlewareAuth(apiCfg.handlerFeedFollowsGet))	
	v1.Delete("/feed_follows/{feedFollowID}",apiCfg.middlewareAuth(apiCfg.handlerFeedFollowDelete))
	router.Mount("/v1",v1)
	srv := &http.Server{
		Handler: router,
		Addr:    ":" + portString,
